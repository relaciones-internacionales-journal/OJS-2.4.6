...
{if not $noIssue}
	...
	<form method="post" action="{url op="updateIssueToc" path=$issueId}" onsubmit="return confirm('{translate|escape:"jsparam" key="editor.issues.saveChanges"}')">
		...
		{foreach from=$sections key=sectionKey item=section}
			...
			<div class="editor-toc-table" id="issueToc-{$sectionKey|escape}">

				<div class="editor-toc-table-head">
				...
				{foreach from=$section[2] item=article name="currSection"}
					...
					<div id="article-{$article->getPublishedArticleId()|escape}" class="editor-toc-table-list">
						...
						<select name="accessStatus[{$article->getPublishedArticleId()}]" size="1" class="selectMenu">{html_options options=$accessOptions selected=$article->getAccessStatus()}</select>
						{/if}
		...
		{if $unpublished && !$isLayoutEditor}
			...
			<input type="button" value="{translate key="editor.issues.publishIssue"}" onclick="confirmAction('{url op="publishIssue" path=$issueId}', '{translate|escape:"jsparam" key="editor.issues.confirmPublish"}')" class="btn btn-sm" />
		{elseif !$isLayoutEditor}
			{* Published; give the option to unpublish it. *}
			<input type="button" value="{translate key="editor.issues.unpublishIssue"}" onclick="confirmAction('{url op="unpublishIssue" path=$issueId}', '{translate|escape:"jsparam" key="editor.issues.confirmUnpublish"}')" class="btn btn-danger btn-sm" />
		{/if}
		<input type="submit" value="{translate key="common.save"}" class="btn btn-sm" /></p>
	</form>
	</div>
{/if}
