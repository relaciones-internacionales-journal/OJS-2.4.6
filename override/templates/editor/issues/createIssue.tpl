...
{if count($formLocales) > 1}
	<div class="form-group">
		<label>{fieldLabel name="formLocale" key="form.formLanguage"}</label>
...
{/if}
	...
	<div class="form-group">
		<label>{fieldLabel name="publicIssueId" key="editor.issues.publicIssueIdentifier"}</label>
		<input type="text" name="publicIssueId" id="publicIssueId" value="{$publicIssueId|escape}" size="20" maxlength="255" class="textField" />
	</div>
	{/if}
	<div class="form-group">
		<label>{fieldLabel name="title" key="issue.title"}</label>
		<input type="text" name="title[{$formLocale|escape}]" id="title" value="{$title[$formLocale]|escape}" size="40" maxlength="255" class="textField" />
	</div>
	<div class="form-group">
		<label>{fieldLabel name="description" key="editor.issues.description"}</label>
...
<div id="issue-cover">
	...
	<div class="form-group">
		<label for="showCoverPage">{translate key="editor.issues.showCoverPage"}</label>
	...
	<div class="form-group">
		<label>{fieldLabel name="coverPage" key="editor.issues.coverPage"}</label>
		<input type="file" name="coverPage" id="coverPage" class="btn btn-file btn-sm" /><br />{translate key="form.saveToUpload"}<br/>
			{translate key="editor.issues.coverPageInstructions"}
	</div>
	<div class="form-group">
		<label>{fieldLabel name="styleFile" key="editor.issues.styleFile"}</label>
		<input type="file" name="styleFile" id="styleFile" class="btn btn-file btn-sm" /><br />{translate key="form.saveToUpload"}<br />{translate key="editor.issues.uploaded"}:&nbsp;{if $styleFileName}<a href="javascript:openWindow('{$publicFilesDir}/{$styleFileName|escape}');" class="file">{$originalStyleFileName|escape}</a>&nbsp;<a href="{url op="removeStyleFile" path=$issueId}" onclick="return confirm('{translate|escape:"jsparam" key="editor.issues.removeStyleFile"}')">{translate key="editor.issues.remove"}</a>{else}&mdash;{/if}
	</div>
	<div class="form-group">
		<label>{fieldLabel name="coverPageDescription" key="editor.issues.coverPageCaption"}</label>
	...
	<div class="form-group">
		<label>{fieldLabel name="hideCoverPageArchives" key="editor.issues.coverPageDisplay"}</label>
...
<div class="save">
	...
	<p><input type="button" value="{translate key="common.cancel"}" onclick="document.location.href='{url op="index" escape=false}'" class="btn btn-danger btn-sm" /> <input type="submit" value="{translate key="common.save"}" class="btn btn-sm"/></p>	
</div>	
</form>
</div>
