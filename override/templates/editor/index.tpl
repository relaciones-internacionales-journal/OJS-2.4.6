...
{/strip}
<div id="editor-home-article-submissions">
<h3>{translate key="article.submissions"}</h3>

<ul class="plain">
	<li class="question-general"><a href="{url op="submissions" path="submissionsUnassigned"}">{translate key="common.queue.short.submissionsUnassigned"}</a>&nbsp;({if $submissionsCount[0]}{$submissionsCount[0]}{else}0{/if})</li>
	<li class="search-general"><a href="{url op="submissions" path="submissionsInReview"}">{translate key="common.queue.short.submissionsInReview"}</a>&nbsp;({if $submissionsCount[1]}{$submissionsCount[1]}{else}0{/if})</li>
	<li class="editar-general"><a href="{url op="submissions" path="submissionsInEditing"}">{translate key="common.queue.short.submissionsInEditing"}</a>&nbsp;({if $submissionsCount[2]}{$submissionsCount[2]}{else}0{/if})</li>
	<li class="document"><a href="{url op="submissions" path="submissionsArchives"}">{translate key="common.queue.short.submissionsArchives"}</a></li>
	{call_hook name="Templates::Editor::Index::Submissions"}
</ul>
</div>

<div id="editor-home-issues">
<h3>{translate key="editor.navigation.issues"}</h3>

<ul class="plain">
	<li class="crear-general"><a href="{url op="createIssue"}">{translate key="editor.navigation.createIssue"}</a></li>
	<li class="notify-general"><a href="{url op="notifyUsers"}">{translate key="editor.notifyUsers"}</a></li>
	<li class="puzzle-general"><a href="{url op="futureIssues"}">{translate key="editor.navigation.futureIssues"}</a></li>
	<li class="folder-general"><a href="{url op="backIssues"}">{translate key="editor.navigation.issueArchive"}</a></li>
...
<div id="editor-home-search">
	...
	<form method="post" id="submit" action="{url path="search"}">
		...
		<br/>
		<input type="submit" value="{translate key="common.search"}" class="btn btn-sm" style="margin-top:20px;"/>
	</form>
&nbsp;
</div>
