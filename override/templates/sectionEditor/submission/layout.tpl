...
<div id="layout">
<h3><a role="button" data-toggle="collapse" href="#tog-four" aria-expanded="false" aria-controls="tog-four">{translate key="submission.layout"} <i class="fa fa-chevron-circle-down"></i></a></h3>
<div class="collapse" id="tog-four">
{if $useLayoutEditors}
<div id="layout-editors">
...
<form method="post" action="{url op="uploadLayoutFile"}"  enctype="multipart/form-data">
	...
	<input type="file" name="layoutFile" size="10" class="btn btn-file btn-sm" />
	<input type="submit" value="{translate key="common.upload"}" class="btn btn-sm" style="margin-top:10px;"/>
	<br /><br />
	...
	<input type="submit" name="createRemote" value="{translate key="common.create"}" class="btn btn-sm" style="margin-left:10px;"/>
	</p>
</form>

<div id="layout-comments">
...
{if $submission->getMostRecentLayoutComment()}
	...
	<a href="javascript:openComments('{url op="viewLayoutComments" path=$submission->getId() anchor=$comment->getId()}');" class="icon"><i class="fa fa-comment"></i></a> {$comment->getDatePosted()|date_format:$dateFormatShort}
{else}
	<a href="javascript:openComments('{url op="viewLayoutComments" path=$submission->getId()}');" class="icon"><i class="fa fa-comment"></i></a> {translate key="common.noComments"}
{/if}

{if $currentJournal->getLocalizedSetting('layoutInstructions')}
&nbsp;&nbsp;
<a href="javascript:openHelp('{url op="instructions" path="layout"}')" class="action question-general">{translate key="submission.layout.instructions"}</a>
{/if}
{if $currentJournal->getSetting('provideRefLinkInstructions')}
&nbsp;&nbsp;
<a href="javascript:openHelp('{url op="instructions" path="referenceLinking"}')" class="action link-general">{translate key="submission.layout.referenceLinking"}</a>
{/if}
