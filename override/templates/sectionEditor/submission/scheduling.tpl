 ...
 *}
<div id="scheduling">
<h3><a role="button" data-toggle="collapse" href="#tog-three" aria-expanded="false" aria-controls="tog-three">{translate key="submission.scheduling"} <i class="fa fa-chevron-circle-down"></i></a></h3>
...
{if !$publicationFeeEnabled || $publicationPayment}
	<form action="{url op="scheduleForPublication" path=$submission->getId()}" method="post">
		<p><strong>{translate key="editor.article.scheduleForPublication"}</strong></p>
			...
			<br/>
				<input type="submit" value="{translate key="common.record"}" class="btn btn-sm" style="margin-top:10px;"/><br/><br/>
				{if $issueId}
					{if $isEditor}
						<a href="{url page="editor" op="issueToc" path=$issueId}" class="action search-general">{translate key="issue.toc"}</a>
					{else}
						<a href="{url page="issue" op="view" path=$issueId}" class="action searcg-general">{translate key="issue.toc"}</a>
					{/if}
				{/if}
