 ...
 *
 *}
<div id="submission-management-review">
	<div class="submission-management-reviewdetails">
		...
		<div class="collapse" id="tog-one">
			...
			<p><strong>{translate key="submission.reviewVersion"}</strong><br/>	
				{if $reviewFile}
					...
					{$reviewFile->getDateModified()|date_format:$dateFormatShort}<br/>{if $currentJournal->getSetting('showEnsuringLink')}<a class="action question-general" href="javascript:openHelp('{get_help_id key="editorial.sectionEditorsRole.review.blindPeerReview" url="true"}')">{translate key="reviewer.article.ensuringBlindReview"}</a>{/if}	
				{else}
			...
			<div class="col-md-6">	
				...
				{foreach from=$suppFiles item=suppFile}
						...
						{/if}			
							...
							{if $suppFile->getFileId() > 0}<a href="{url op="downloadFile" path=$submission->getId()|to_array:$suppFile->getFileId():$suppFile->getRevision()}" class="file descargar-general">{$suppFile->getFileName()|escape}</a>&nbsp;&nbsp;
							{$suppFile->getDateModified()|date_format:$dateFormatShort}
							{elseif $suppFile->getRemoteURL() != ''}<a href="{$suppFile->getRemoteURL()|escape}" target="_blank">{$suppFile->getRemoteURL()|truncate:20:"..."|escape}</a>
							{/if}
							<br/>
							...
							<br/><input type="submit" name="submit" value="{translate key="common.record"}" class="btn btn-sm" />
							</form>		
					{foreachelse}
					<p><strong>{translate key="article.suppFilesAbbrev"}</strong><br/>
					{translate key="common.none"}
...
<div id="peer-review">
	<h3><a role="button" data-toggle="collapse" href="#tog-two" aria-expanded="false" aria-controls="tog-two">{translate key="submission.peerReview"} <i class="fa fa-chevron-circle-down"></i></a></h3>
	...
	<table class="data-alt" width="100%">
		<tr id="reviewersHeader" valign="middle">
			...
			<td width="64%" class="nowrap">
				<a href="{url op="selectReviewer" path=$submission->getId()}" class="action assign-general">{translate key="editor.article.selectReviewer"}</a>&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="{url op="submissionRegrets" path=$submission->getId()}" class="action common-preview">{translate|escape key="sectionEditor.regrets.link"}</a>
			</td>
		</tr>
	</table>
...
{if not $reviewAssignment->getCancelled() and not $reviewAssignment->getDeclined()}
	...
	{if $reviewAssignment->getDateConfirmed() && !$reviewAssignment->getDeclined()}
		<p><strong>{translate key="reviewer.article.recommendation"}: </strong><br/>
			...
			{else}
				{translate key="common.none"}&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="{url op="remindReviewer" articleId=$submission->getId() reviewId=$reviewAssignment->getId()}" class="action notify-general">{translate key="reviewer.article.sendReminder"}</a>
		...
		{if $reviewFormResponses[$reviewId]}
			<p><strong>{translate key="submission.reviewFormResponse"}: </strong><br/>
				<a href="javascript:openComments('{url op="viewReviewFormResponse" path=$submission->getId()|to_array:$reviewAssignment->getId()}');" class="document"></a>
			</p>
		{/if}
		{if !$reviewAssignment->getReviewFormId() || $reviewAssignment->getMostRecentPeerReviewComment()}{* Only display comments link if a comment is entered or this is a non-review form review *}
			<p><strong>{translate key="submission.review"}: </strong><br/>
				{if $reviewAssignment->getMostRecentPeerReviewComment()}
					...
					<a href="javascript:openComments('{url op="viewPeerReviewComments" path=$submission->getId()|to_array:$reviewAssignment->getId() anchor=$comment->getId()}');" class="icon"><i class="fa fa-comment"></i></a>&nbsp;&nbsp;{$comment->getDatePosted()|date_format:$dateFormatShort}
				{else}
					<a href="javascript:openComments('{url op="viewPeerReviewComments" path=$submission->getId()|to_array:$reviewAssignment->getId()}');" class="icon"><i class="fa fa-comment"></i></a>&nbsp;&nbsp;{translate key="submission.comments.noComments"}
				{/if}
			</p>
		{/if}
	...
	{if (($reviewAssignment->getRecommendation() === null || $reviewAssignment->getRecommendation() === '') || !$reviewAssignment->getDateConfirmed()) && $reviewAssignment->getDateNotified() && !$reviewAssignment->getDeclined()}
		<p><strong>{translate key="reviewer.article.editorToEnter"}: </strong><br/>
				...
				{if $reviewAssignment->getDateConfirmed() && !$reviewAssignment->getDeclined()}
					<br/><a class="action crear-general" href="{url op="enterReviewerRecommendation" articleId=$submission->getId() reviewId=$reviewAssignment->getId()}">{translate key="editor.article.recommendation"}</a>
				{/if}
		</p>
	{/if}

	{if $reviewAssignment->getDateNotified() && !$reviewAssignment->getDeclined() && $rateReviewerOnQuality}
		<p><strong>{translate key="editor.article.rateReviewer"}</strong></p>
		<form method="post" action="{url op="rateReviewer"}">
			...
			<input type="submit" value="{translate key="common.record"}" class="btn btn-sm" style="margin-top:10px;"/>
