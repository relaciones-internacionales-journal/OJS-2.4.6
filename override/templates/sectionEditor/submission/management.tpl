...
{assign var="suppFiles" value=$submission->getSuppFiles()}
	...
	<p><strong>{translate key="submission.originalFile"}</strong><br/>
			{if $submissionFile}
				<a href="{url op="downloadFile" path=$submission->getId()|to_array:$submissionFile->getFileId()}" class="file descargar-general">{$submissionFile->getFileName()|escape}</a>&nbsp;&nbsp;{$submissionFile->getDateModified()|date_format:$dateFormatShort}
			{else}
	...
	<p><strong>{translate key="article.suppFilesAbbrev"}</strong><br/>
			{foreach name="suppFiles" from=$suppFiles item=suppFile}
				{if $suppFile->getFileId()}
					<a href="{url op="downloadFile" path=$submission->getId()|to_array:$suppFile->getFileId()}" class="file descargar-general">{$suppFile->getFileName()|escape}</a>
					&nbsp;&nbsp;
				...
				{/if}
				<a href="{url op="editSuppFile" from="submission" path=$submission->getId()|to_array:$suppFile->getId()}" class="action editar-general">{translate key="common.edit"}</a>
				&nbsp;|&nbsp;
				...
				{if !$notFirst}
					<br/>
					<a href="{url op="addSuppFile" from="submission" path=$submission->getId()}" class="action crear-general">{translate key="submission.addSuppFile"}</a>
				{/if}
				<br />
				{assign var=notFirst value=1}
			{foreachelse}
				{translate key="common.none"}&nbsp;&nbsp;&nbsp;&nbsp;<a href="{url op="addSuppFile" from="submission" path=$submission->getId()}" class="action crear-general">{translate key="submission.addSuppFile"}</a>
			{/foreach}
	</p>
	...
	<p><strong>{translate key="section.section"}</strong><br/>
		...
		<select name="section" size="1" class="selectMenu input-medium" style="margin-bottom:10px;">{html_options options=$sections selected=$submission->getSectionId()}</select> <br/>
		<input type="submit" value="{translate key="common.record"}" class="btn btn-sm" /></form>
	</p>
