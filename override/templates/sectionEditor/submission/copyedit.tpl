 ...
 *}
<div id="copyedit">
<h3><a role="button" data-toggle="collapse" href="#tog-two" aria-expanded="false" aria-controls="tog-two">{translate key="submission.copyediting"} <i class="fa fa-chevron-circle-down"></i></a></h3>
<div class="collapse" id="tog-two">
{if $currentJournal->getLocalizedSetting('copyeditInstructions')}
&nbsp;&nbsp;
<a href="javascript:openHelp('{url op="instructions" path="copy"}')" class="action question-general">{translate key="submission.copyedit.instructions"}</a>
{/if}
...
<table width="100%" class="data-alt">
	<tr>
		...
		<td class="value"><a href="{url op="selectCopyeditor" path=$submission->getId()}" class="action question-general">{translate key="editor.article.selectCopyeditor"}</a></td>
	</tr>
</table>
{/if}
...
<table width="100%" class="info">
	<tr>
		<td width="28%" colspan="2"><a href="{url op="viewMetadata" path=$submission->getId()}" class="action editar-general">{translate key="submission.reviewMetadata"}</a></td>
	...
	<tr>
		...
		<td colspan="5">
			...
			{if $initialCopyeditFile}
				<a href="{url op="downloadFile" path=$submission->getId()|to_array:$initialCopyeditFile->getFileId():$initialCopyeditFile->getRevision()}" class="file descargar-general">{$initialCopyeditFile->getFileName()|escape}</a>&nbsp;&nbsp;{$initialCopyeditFile->getDateModified()|date_format:$dateFormatShort}
			{else}
	...
	<tr>
		...
		<td colspan="5">
			...
			{if $editorAuthorCopyeditFile}
				<a href="{url op="downloadFile" path=$submission->getId()|to_array:$editorAuthorCopyeditFile->getFileId():$editorAuthorCopyeditFile->getRevision()}" class="file descargar-general">{$editorAuthorCopyeditFile->getFileName()|escape}</a>&nbsp;&nbsp;{$editorAuthorCopyeditFile->getDateModified()|date_format:$dateFormatShort}
			{/if}
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td colspan="5">
			...
			{if $finalCopyeditFile}
				<a href="{url op="downloadFile" path=$submission->getId()|to_array:$finalCopyeditFile->getFileId():$finalCopyeditFile->getRevision()}" class="file descargar-general">{$finalCopyeditFile->getFileName()|escape}</a>&nbsp;&nbsp;{$finalCopyeditFile->getDateModified()|date_format:$dateFormatShort}
			{/if}
		</td>
	</tr>
	<tr>
...
<form method="post" action="{url op="uploadCopyeditVersion"}"  enctype="multipart/form-data">
	...
	<input type="file" name="upload" size="10" class="btn btn-file btn-sm"{if !$canUploadCopyedit} disabled="disabled"{/if} />{translate key="plugins.block.navigation.journalContent"}
	<input type="submit" value="{translate key="common.upload"}" class="btn btn-sm" style="margin-top:10px;" {if !$canUploadCopyedit} disabled="disabled"{/if} />
	</p>
</form>
...
{if $submission->getMostRecentCopyeditComment()}
	...
	<a href="javascript:openComments('{url op="viewCopyeditComments" path=$submission->getId() anchor=$comment->getId()}');" class="icon"><i class="fa fa-comment"></i></a>{$comment->getDatePosted()|date_format:$dateFormatShort}
{else}
	<a href="javascript:openComments('{url op="viewCopyeditComments" path=$submission->getId()}');" class="icon"><i class="fa fa-comment"></i></a>{translate key="common.noComments"}
{/if}
