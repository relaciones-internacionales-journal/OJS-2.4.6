...
<div id="proofread">
<h3><a role="button" data-toggle="collapse" href="#tog-five" aria-expanded="false" aria-controls="tog-five">{translate key="submission.proofreading"} <i class="fa fa-chevron-circle-down"></i></a></h3>
...
{if $submission->getMostRecentProofreadComment()}
	...
	<a href="javascript:openComments('{url op="viewProofreadComments" path=$submission->getId() anchor=$comment->getId()}');" class="icon"><i class="fa fa-comment"></i></a>{$comment->getDatePosted()|date_format:$dateFormatShort}
{else}
	<a href="javascript:openComments('{url op="viewProofreadComments" path=$submission->getId()}');" class="icon"><i class="fa fa-comment"></i></a>{translate key="common.noComments"}
{/if}

{if $currentJournal->getLocalizedSetting('proofInstructions')}
&nbsp;&nbsp;
<a href="javascript:openHelp('{url op="instructions" path="proof"}')" class="action question-general">{translate key="submission.proofread.instructions"}</a>
{/if}
</div>
</div>
