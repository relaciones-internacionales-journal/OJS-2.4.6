...
<form id="submit" method="post" action="{url op="selectReviewer" path=$articleId}">
	...
	<input type="text" size="10" name="search" class="textField" value="{$search|escape}" />&nbsp;<input type="submit" value="{translate key="common.search"}" class="btn btn-sm" />
</form>
...
<p><a class="action assign-general" href="{url op="enrollSearch" path=$articleId}">{translate key="sectionEditor.review.enrollReviewer"}</a><br/>
<a class="action crear-general" href="{url op="createReviewer" path=$articleId}">{translate key="sectionEditor.review.createReviewer"}</a>{foreach from=$reviewerDatabaseLinks item="link"}{if !empty($link.title) && !empty($link.url)}&nbsp;|&nbsp;<a href="{$link.url|escape}" target="_new" class="action">{$link.title|escape}</a>{/if}{/foreach}</p>
<hr/>
...
<tr class="heading" valign="bottom">
	...
	<td style="text-align:left">{translate key="user.interests"}</td>
	{if $rateReviewerOnQuality}
...
<tr valign="top">
	<td style="text-align:left"><a class="action" href="{url op="userProfile" path=$userId}">{$reviewer->getFullName()|escape}</a></td>
	<td style="text-align:left">{$reviewer->getInterestString()|escape}</td>
	...
	<td style="text-align:center">
		{if $reviewer->review_id}
			{if $reviewer->declined}
				<a class="action assign-general" href="{url op="reassignReviewer" path=$articleId|to_array:$reviewer->getUserId()}">{translate key="editor.reassign"}</a>
			{else}
				{translate key="common.alreadyAssigned"}
			{/if}
		{else}
		<a class="action assign-general" href="{url op="selectReviewer" path=$articleId|to_array:$reviewer->getId()}">{translate key="common.assign"}</a>
		{/if}
	</td>
</tr>
