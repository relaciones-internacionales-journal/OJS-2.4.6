...
{else}
	<tr>
		<td colspan="3" align="left" class="table-results-pagination">{page_info iterator=$emailLogEntries}</td>
		<td colspan="2" align="right" class="table-results-pagination">{page_links anchor="emailLogEntries" name="emailLogEntries" iterator=$emailLogEntries}</td>
	</tr>
{/if}
</table>

{if $isEditor}
<a class="action delete" href="{url page="editor" op="clearSubmissionEmailLog" path=$submission->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="submission.email.confirmClearLog"}')">{translate key="submission.history.clearLog"}</a>
{/if}
</div>
