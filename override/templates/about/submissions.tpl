...
{foreach from=$sections item=section}{if !$section->getHideAbout()}
	...
	<table class="plain" width="60%">
		<tr>
			<td>{if !$section->getEditorRestricted()}<p class="icon-checked">{translate key="manager.sections.open"}</p>{else}<p class="icon-unchecked">{translate key="manager.sections.open"}</p>{/if} </td>
			<td>{if $section->getMetaIndexed()}<p class="icon-checked">{translate key="manager.sections.indexed"}</p>{else}<p class="icon-unchecked">{translate key="manager.sections.indexed"}{/if}</td>
			<td>{if $section->getMetaReviewed()}<p class="icon-checked">{translate key="manager.sections.reviewed"}</p>{else}<p class="icon-unchecked">{translate key="manager.sections.reviewed"}{/if}</td>
		</tr>
	</table>
{/if}{/foreach}
</div>
