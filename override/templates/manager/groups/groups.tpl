...
{iterate from=groups item=group}
	{if $group->getContext() == GROUP_CONTEXT_EDITORIAL_TEAM}
		...
		{/if}
		<td>
			<a href="{url op="editGroup" path=$group->getId()}" class="action  editar-general">{translate key="common.edit"}</a>&nbsp;|&nbsp;<a href="{url op="groupMembership" path=$group->getId()}" class="action add-user">{translate key="manager.groups.membership"}</a>&nbsp;|&nbsp;<a href="{url op="deleteGroup" path=$group->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="manager.groups.confirmDelete"}')" class="action delete">{translate key="common.delete"}</a>&nbsp;|&nbsp;<a href="{url op="moveGroup" d=u id=$group->getId()}">&uarr;</a>&nbsp;<a href="{url op="moveGroup" d=d id=$group->getId()}">&darr;</a>
		</td>
	</tr>
	<tr>
...
{else}
	<tr>
		<td colspan="2" align="left" class="table-results-pagination">{page_info iterator=$groups}</td>
		<td colspan="1" align="right" class="table-results-pagination">{page_links anchor="groups" name="groups" iterator=$groups}</td>
	</tr>
{/if}
</table>

<a href="{url op="createGroup"}" class="action crear-general">{translate key="manager.groups.create"}</a>
</div>
