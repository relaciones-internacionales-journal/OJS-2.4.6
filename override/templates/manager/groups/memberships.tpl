...
{else}
	...
	<tr>
		<td align="left" class="table-results-pagination">{page_info iterator=$memberships}</td>
		<td align="right" class="table-results-pagination">{page_links anchor="membership" name="memberships" iterator=$memberships}</td>
	</tr>
{/if}
</table>

<a href="{url op="addMembership" path=$group->getId()}" class="action crear-general">{translate key="manager.groups.membership.addMember"}</a>
</div>
