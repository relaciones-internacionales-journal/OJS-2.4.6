		...
		{foreach from=$plugins item=plugin}
			...
			{elseif $managementVerbs}
				{foreach from=$managementVerbs item=verb}
					{if $verb[0] == 'settings'}
						<a class="action settings-cog" href="{url op="plugin" path=$category|to_array:$plugin->getName():$verb[0]}">{$verb[1]|escape}</a>&nbsp;
					{elseif $verb[0] == 'enable'}
						<a class="action turn-on" href="{url op="plugin" path=$category|to_array:$plugin->getName():$verb[0]}">{$verb[1]|escape}</a>&nbsp;
					{elseif $verb[0] == 'disable'}
						<a class="action turn-off" href="{url op="plugin" path=$category|to_array:$plugin->getName():$verb[0]}">{$verb[1]|escape}</a>&nbsp;
					{else}
			...
			{if $plugin->getCurrentVersion() && !$preventManagerPluginManagement}
				...
				<a class="action update-refresh" href="{url op="managePlugins" path="upgrade"|to_array:$category:$pluginInstallName}">{translate key="manager.plugins.upgrade"}</a>&nbsp;
