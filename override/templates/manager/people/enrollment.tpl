...
<form method="post" action="{url path=$roleSymbolic}">
	...
	<input type="text" size="10" name="search" class="textField" value="{$search|escape}" /><br/><input type="submit" value="{translate key="common.search"}" class="btn btn-sm" />
</form>
...
<table width="100%" class="listing">
	...
	<tr valign="top">
		...
		<td align="right">
			{if $roleId}
			<a href="{url op="unEnroll" path=$roleId userId=$user->getId() journalId=$currentJournal->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="manager.people.confirmUnenroll"}')" class="action ban">{translate key="manager.people.unenroll"}</a>&nbsp;|
			{/if}
			<a href="{url op="editUser" path=$user->getId()}" class="action editar-general">{translate key="common.edit"}</a>
			{if $thisUser->getId() != $user->getId()}
				|&nbsp;<a href="{url page="login" op="signInAsUser" path=$user->getId()}" class="action settings-cog">{translate key="manager.people.signInAs"}</a>
				{if !$roleId}|&nbsp;<a href="{url op="removeUser" path=$user->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="manager.people.confirmRemove"}')" class="action delete">{translate key="manager.people.remove"}</a>{/if}
				{if $user->getDisabled()}
					|&nbsp;<a href="{url op="enableUser" path=$user->getId()}" class="action turn-on">{translate key="manager.people.enable"}</a>
				{else}
					|&nbsp;<a href="javascript:confirmAndPrompt({$user->getId()})" class="action turn-off">{translate key="manager.people.disable"}</a>
				{/if}
			{/if}
		</td>
	</tr>
...
{if $userExists}
	...
	<p><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-sm" onclick="document.location.href='{url page="manager" escape=false}'" />&nbsp;<input type="button" value="{translate key="common.selectAll"}" class="btn btn-sm" onclick="toggleChecked()" />&nbsp;<input type="submit" value="{translate key="email.compose"}" class="btn btn-sm"/></p>
{/if}
</div>
</form>

<a href="{url op="enrollSearch" path=$roleId}" class="action crear-general">{translate key="manager.people.enrollExistingUser"}</a> |
...
<a href="{if $roleId}{url op="createUser" roleId=$roleId source=$enrollmentUrl}{else}{url op="createUser" source=$enrollmentUrl}{/if}" class="action crear-general">{translate key="manager.people.createUser"}</a> | <a href="{url op="enrollSyncSelect" path=$rolePath}" class="action update-refresh">{translate key="manager.people.enrollSync"}</a>
</div>
