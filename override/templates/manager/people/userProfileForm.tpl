...
{/if}
	...
	<tr valign="top">
		...
		<td class="value">
			<input type="text" name="username" id="username" value="{$username|escape}" size="20" maxlength="32" class="textField" />&nbsp;&nbsp;<input type="button" class="btn btn-sm" value="{translate key="common.suggest"}" onclick="generateUsername()" />
			<br />
...
<hr/>
<p style="text-align:right;"><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-sm" onclick="{if $source == ''}history.go(-1);{else}document.location='{$source|escape:"jsparam"}';{/if}" /> {if not $userId}<input type="submit" name="createAnother" value="{translate key="manager.people.saveAndCreateAnotherUser"}" class="btn btn-sm" /> {/if} <input type="submit" value="{translate key="common.save"}" class="btn btn-sm" /></p>
