...
{if not $omitSearch}
	...
	<input type="hidden" name="roleId" value="{$roleId|escape}"/>
		...
		<input type="text" size="15" name="search" class="textField" value="{$search|escape}" />&nbsp;<input type="submit" value="{translate key="common.search"}" class="btn btn-sm" />
	</form>
...
<tr valign="top">
	...
	<td align="right" class="nowrap">
		{if $roleId}
		<a href="{url op="enroll" path=$roleId userId=$user->getId()}" class="action crear-general">{translate key="manager.people.enroll"}</a>
		{else}
		<a href="#" onclick="enrollUser({$user->getId()})" class="action crear-general">{translate key="manager.people.enroll"}</a>
		{/if}
...
{else}
	<tr>
		<td colspan="3" align="left" class="table-results-pagination">{page_info iterator=$users}</td>
		<td colspan="2" align="right" class="table-results-pagination">{page_links anchor="users" name="users" iterator=$users searchInitial=$searchInitial searchField=$searchField searchMatch=$searchMatch search=$search dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateFromMonth=$dateFromMonth dateToDay=$dateToDay dateToYear=$dateToYear dateToMonth=$dateToMonth roleId=$roleId sort=$sort sortDirection=$sortDirection}</td>
	</tr>
{/if}
</table>
</div>
<div id="separator"><hr/></div>
<p style="text-align-right;"><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-sm" onclick="document.location.href='{url page="manager" escape=false}'" /> <input type="button" value="{translate key="common.selectAll"}" class="btn btn-sm" onclick="toggleChecked()" /> <input type="submit" value="{translate key="manager.people.enrollSelected"}" class="btn btn-sm" /></p>
