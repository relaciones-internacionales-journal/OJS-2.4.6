...
{/if}
	<tr valign="top" id="reviewform-{$reviewForm->getId()}" class="data">
		...
		<td align="right" class="nowrap">
			{if $canEdit}<a href="{url op="editReviewForm" path=$reviewForm->getId()}" class="action editar-general">{translate key="common.edit"}</a>&nbsp;|{/if}
			{strip}
				{if $reviewForm->getActive()}
					<a href="{url op="deactivateReviewForm" path=$reviewForm->getId()}" class="action  turn-off">{translate key="common.deactivate"}</a>
				{else}
					<a href="{url op="activateReviewForm" path=$reviewForm->getId()}" class="action  turn-on">{translate key="common.activate"}</a>
				{/if}
				&nbsp;|
			{/strip}
			{if !$canEdit}<a href="{url op="copyReviewForm" path=$reviewForm->getId()}" class="action copiar-general">{translate key="common.copy"}</a>&nbsp;|{/if}
			<a href="{url op="previewReviewForm" path=$reviewForm->getId()}" class="action common-preview">{translate key="common.preview"}</a>&nbsp;|
			{if $canEdit}<a href="{url op="deleteReviewForm" path=$reviewForm->getId()}" onclick="return confirm('{translate|escape:"jsparam" key="manager.reviewForms.confirmDeleteUnpublished"}')" class="action delete">{translate key="common.delete"}</a>&nbsp;|{/if}
...
{else}
	<tr>
		<td colspan="2" align="left" class="table-results-pagination">{page_info iterator=$reviewForms}</td>
		<td colspan="2" align="right" class="table-results-pagination">{page_links anchor="reviewForms" name="reviewForms" iterator=$reviewForms}</td>
	</tr>
{/if}

</table>

<a class="action crear-general" href="{url op="createReviewForm"}">{translate key="manager.reviewForms.create"}</a>
</div>
