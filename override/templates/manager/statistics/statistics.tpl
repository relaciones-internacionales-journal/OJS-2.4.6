...
<form action="{url op="saveStatisticsSettings"}" method="post">
	{if count($availableMetricTypes) > 1}
		...
		<div id="default-metric-selection">
...
<table width="100%" class="data-alt">
	...
	<tr valign="top">
		<td width="20%" class="label"><label class="checkbox" for="statCountRevise"><input type="checkbox" id="statCountRevise" name="statCountRevise" {if $statCountRevise}checked="checked" {/if}/>&nbsp;&nbsp;{translate key="manager.statistics.statistics.count.revise"}</label></td>
