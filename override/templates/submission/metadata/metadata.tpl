...
<div class="edit-metadata">
	<p><a href="{url op="viewMetadata" path=$submission->getId()}" class="action editar-general">{translate key="submission.editMetadata"}</a></p>
...
<h4>{translate key="article.authors"}</h4>	
	...
	{if $author->getData('orcid')}
		<p><strong>{translate key="user.orcid"}</strong>: <a href="{$author->getData('orcid')|escape}" target="_blank">{$author->getData('orcid')|escape}</a></a>
		</p>
	{/if}
	{if $author->getUrl()}
