...
{/if}
<div id="reviewer-review-form-response">
...
<form id="saveReviewFormResponse" method="post" action="{url op="saveReviewFormResponse" path=$reviewId|to_array:$reviewForm->getId()}">
	...
	{if $editorPreview}	
		<p style="text-align:right;"><input type="button" value="{translate key="common.close"}" class="btn btn-danger btn-sm" onclick="window.close()" /></p>
	{else}
		<p style="text-align:right;"><input type="button" value="{translate key="common.close"}" class="btn btn-danger btn-sm" onclick="document.location.href='{url op="submission" path=$reviewId}'" /> <input {if $disabled}disabled="disabled" {/if}type="submit" name="save" value="{translate key="common.save"}" class="btn btn-sm" /></p>
	{/if}
