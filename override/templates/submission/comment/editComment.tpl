...
<p><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-sm" onclick="history.go(-1);" /> {if !$isPeerReviewComment}<input type="submit" name="saveAndEmail" value="{translate key="common.saveAndEmail"}" class="btn btn-sm" />{/if} <input type="submit" value="{translate key="common.save"}" class="btn btn-sm" /></p>
