	...
	<form method="post" id="searchForm" action="{url op="search"}">
		...
		{if $hasEmptyFilters}
			{capture assign="emptyFilters"}				
					...
					<div class="col-2">
						<div class="search-filters">
							<div class="arrow-down-box"><h4>{translate key="search.indexTerms"}</h4>
							</div>
						</div>
				...
				<div class="date-search">
					<div class="search-filters">
						<div class="arrow-down-box"><h4>{translate key="search.date"}</h4>
						</div>
					</div>
...
<div id="advanced-search-results">
	<div class="search-filters">
		<div class="arrow-down-box"><h4>{translate key="search.infoSearchResults"}</h4>
			</div>
		</div>
	<table width="100%" class="listing" style="font-size:12px;">
		...
		{else}
			<tr>
				<td class="table-results-pagination"{if !$currentJournal}colspan="2" {/if}align="left">{page_info iterator=$results}</td>
				<td class="table-results-pagination-pages" colspan="2" align="right">{page_links anchor="results" iterator=$results name="search" query=$query searchJournal=$searchJournal authors=$authors title=$title abstract=$abstract galleyFullText=$galleyFullText suppFiles=$suppFiles discipline=$discipline subject=$subject type=$type coverage=$coverage indexTerms=$indexTerms dateFromMonth=$dateFromMonth dateFromDay=$dateFromDay dateFromYear=$dateFromYear dateToMonth=$dateToMonth dateToDay=$dateToDay dateToYear=$dateToYear orderBy=$orderBy orderDir=$orderDir}</td>
			</tr>
		{/if}
	</table>
</div>
...
{* MODIFICADO OJS V.2.4.6 / 06-2015/ Missing an ending </div>? *}