...
<div id="submissionToBeReviewed">
	...
	<p><strong>{translate key="submission.metadata"}: </strong><a href="{url op="viewMetadata" path=$reviewId|to_array:$articleId}" class="action buscar-general" target="_new">{translate key="submission.viewMetadata"}</a>
	</p>
...
{assign var="currentStep" value=1}
	...
	<p><strong>{$currentStep|escape}.{assign var="currentStep" value=$currentStep+1}</strong>&nbsp;
		<span class="help-block">{translate key="reviewer.article.notifyEditorA"}{if $firstEditAssignment}, {$firstEditAssignment->getEditorFullName()|escape},{/if} {translate key="reviewer.article.notifyEditorB"}</span>
		<br/>
	...
	{if $journal->getLocalizedSetting('reviewGuidelines') != ''}
		<p><strong>{$currentStep|escape}.{assign var="currentStep" value=$currentStep+1}</strong>&nbsp;
			<span class="help-block">{translate key="reviewer.article.consultGuidelines"}</span>
		</p>
		<hr/>
	{/if}

	<p><strong>{$currentStep|escape}.{assign var="currentStep" value=$currentStep+1}</strong>&nbsp;
		<span class="help-block">{translate key="reviewer.article.downloadSubmission"}</span>
	</p>
		...
		{if ($confirmedStatus and not $declined) or not $journal->getSetting('restrictReviewerFileAccess')}
			<li><strong>{translate key="submission.submissionManuscript"}: </strong>&nbsp;
				...
				{if $submission->getDateConfirmed() or not $journal->getSetting('restrictReviewerAccessToFile')}
					<a href="{url op="downloadFile" path=$reviewId|to_array:$articleId:$reviewFile->getFileId():$reviewFile->getRevision()}" class="file descargar-general">{$reviewFile->getFileName()|escape}</a>
			...
			<li><strong>{translate key="article.suppFiles"}: </strong>&nbsp;
				...
				{foreach from=$suppFiles item=suppFile}
					{if $suppFile->getShowReviewers() }
						...
						<a href="{url op="downloadFile" path=$reviewId|to_array:$articleId:$suppFile->getFileId()}" class="file descargar-general">{$suppFile->getFileName()|escape}</a><br />
					{/if}
				{/foreach}
	...
	{if $currentJournal->getSetting('requireReviewerCompetingInterests')}
		<p><strong>{$currentStep|escape}.{assign var="currentStep" value=$currentStep+1}</strong>&nbsp;
			...
			<span class="help-block">{translate key="reviewer.article.enterCompetingInterests" competingInterestGuidelinesUrl=$competingInterestGuidelinesUrl}</span>
	...
	{if $reviewAssignment->getReviewFormId()}
		<p><strong>{$currentStep|escape}.{assign var="currentStep" value=$currentStep+1}</strong>&nbsp;
			<span class="help-block">{translate key="reviewer.article.enterReviewForm"}</span><br/>
	...
	{else}{* $reviewAssignment->getReviewFormId() *}
		<p><strong>{$currentStep|escape}.{assign var="currentStep" value=$currentStep+1}</strong>&nbsp;
			<span class="help-block">{translate key="reviewer.article.enterReviewA"}</span><br/>
	...
	<p><strong>{$currentStep|escape}.{assign var="currentStep" value=$currentStep+1}</strong>&nbsp;
		<span class="help-block">{translate key="reviewer.article.uploadFile"}</span><br/>
		{foreach from=$submission->getReviewerFileRevisions() item=reviewerFile key=key}
			{assign var=uploadedFileExists value="1"}
				...
				{/if}
				<a href="{url op="downloadFile" path=$reviewId|to_array:$articleId:$reviewerFile->getFileId():$reviewerFile->getRevision()}" class="file descargar-general">{$reviewerFile->getFileName()|escape}</a>
		...
		{if $submission->getRecommendation() === null || $submission->getRecommendation() === ''}
			...
			{if $currentJournal->getSetting('showEnsuringLink')}
				<span class="help-block">
					<br/><a class="action info-general" href="javascript:openHelp('{get_help_id key="editorial.sectionEditorsRole.review.blindPeerReview" url="true"}')">{translate key="reviewer.article.ensuringBlindReview"}</a>
				</span>
			{/if}
		{/if}
	</p>
	...
	<p><strong>{$currentStep|escape}.{assign var="currentStep" value=$currentStep+1}</strong>&nbsp;
		<span class="help-block">{translate key="reviewer.article.selectRecommendation"}</span><br/>
