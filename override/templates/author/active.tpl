...
{iterate from=submissions item=submission}
	...
	<tr valign="top">
		...
		{else}
			...
			<td align="right">{translate key="submissions.incomplete"}<br /><a href="{url op="deleteSubmission" path=$articleId}" class="action delete" onclick="return confirm('{translate|escape:"jsparam" key="author.submissions.confirmDelete"}')">{translate key="common.delete"}</a></td>
		{/if}
...
{else}
	<tr>
		<td colspan="4" align="left" class="table-results-pagination">{page_info iterator=$submissions}</td>
		<td colspan="2" align="right" class="table-results-pagination">{page_links anchor="submissions" name="submissions" iterator=$submissions sort=$sort sortDirection=$sortDirection}</td>
	</tr>
{/if}
</table>
</div>
