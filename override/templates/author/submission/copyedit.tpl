...
<p><a href="javascript:openHelp('{url op="instructions" path="copy"}')" class="action question-general">{translate key="submission.copyedit.instructions"}</a></p>
{/if}
...
<table width="100%" class="info">
	<tr>
		<td width="40%" colspan="2"><a href="{url op="viewMetadata" path=$submission->getId()}" class="action editar-general">{translate key="submission.reviewMetadata"}</a></td>
	...
	<tr>
		...
		<td colspan="5">
			...
			<form method="post" action="{url op="uploadCopyeditVersion"}"  enctype="multipart/form-data">
				...
				<div>
					<input type="file" name="upload"{if not $copyeditAuthorSignoff->getDateNotified() or $copyeditAuthorSignoff->getDateCompleted()} disabled="disabled"{/if} class="btn btn-file btn-sm" />
					<input type="submit" class="btn btn-sm" value="{translate key="common.upload"}"{if not $copyeditAuthorSignoff->getDateNotified() or $copyeditAuthorSignoff->getDateCompleted()} disabled="disabled"{/if} />
				</div>
			</form>
		</td>
	</tr>
