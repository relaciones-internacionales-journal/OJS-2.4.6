...
<form id="submit-form" method="post" action="{url op="saveSubmit" path=$submitStep}" enctype="multipart/form-data">
...
<div>
	<input type="file" name="uploadSuppFile" id="uploadSuppFile"  class="btn btn-file btn-sm"" />
	<input name="submitUploadSuppFile" id="upload-supp-file" type="submit" class="btn btn-sm" value="{translate key="common.upload"}" />
</div>
{if $currentJournal->getSetting('showEnsuringLink')}<br/><br/><a class="action question-general" href="javascript:openHelp('{get_help_id key="editorial.sectionEditorsRole.review.blindPeerReview" url="true"}')">{translate key="reviewer.article.ensuringBlindReview"}</a>{/if}

<div class="separator"><hr/></div>

<p style="text-align:right;"><input type="button" value="{translate key="common.cancel"}" class="btn btn-danger btn-sm" onclick="confirmAction('{url page="author"}', '{translate|escape:"jsparam" key="author.submit.cancelSubmission"}')" /> <input type="submit" onclick="return confirmForgottenUpload()" value="{translate key="common.saveAndContinue"}" class="btn btn-sm" /></p>
